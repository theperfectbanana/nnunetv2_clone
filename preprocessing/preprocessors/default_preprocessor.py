...
# This is non-zero detector delete when you finish debugging, this funcion is NOT a part of default codes.
# DELETE WHEN YOU DONE
def print_nonzero_elements(arr):
    if arr is None:
        print("Array is None")
        return
...
class DefaultPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], dose: Union[np.ndarray, None], properties: dict,
                 plans_manager: PlansManager, configuration_manager: ConfigurationManager,
                 dataset_json: Union[dict, str]):
        # let's not mess up the inputs!
        data = np.copy(data)
        ...
        if dose is not None:
            ...
            assert data.shape[1:] == dose.shape[1:], "Shape mismatch between image and dose. Please fix your dataset and make use of the --verify_dataset_integrity flag to ensure everything is correct"
            dose = np.copy(dose)
        ...
        if seg is not None:
            seg = seg.transpose([0, *[i + 1 for i in plans_manager.transpose_forward]])
        if dose is not None:
            dose = dose.transpose([0, *[i + 1 for i in plans_manager.transpose_forward]])
            # print_nonzero_elements(dose)
        original_spacing = [properties['spacing'][i] for i in plans_manager.transpose_forward]
        ...
        data, seg, dose, bbox = crop_to_nonzero(data, seg, dose)
        print("Dose Cropping finished!")
        # print(dose)
        properties['bbox_used_for_cropping'] = bbox
        properties['shape_after_cropping_and_before_resampling'] = data.shape[1:]
        ...
        if dose is not None:
            dose = self._normalize(data, dose, configuration_manager, plans_manager.foreground_intensity_properties_per_channel)
            print("Dose Normaliztion finished!")
            # print_nonzero_elements(dose)
        ...
        seg = configuration_manager.resampling_fn_seg(seg, new_shape, original_spacing, target_spacing)
        if dose is not None:
            dose = configuration_manager.resampling_fn_seg(dose, new_shape, original_spacing, target_spacing)
            print("Dose resampling finished!")
            # print_nonzero_elements(dose)
        if self.verbose:
            print(f'old shape: {old_shape}, new_shape: {new_shape}, old_spacing: {original_spacing}, '
                  f'new_spacing: {target_spacing}, fn_data: {configuration_manager.resampling_fn_data}')
        ...
        return data, seg, dose


    def run_case(self, image_files: List[str], seg_file: Union[str, None], dose_file: Union[str, None], plans_manager: PlansManager,
        ...
        if seg_file is not None:
            # print("seg file is not none")
            seg, _ = rw.read_seg(seg_file)
            # print(seg)
        else:
            seg = None

        # if possible, load dose
        if dose_file is not None:
            dose, _ = rw.read_dose(dose_file)
            # print(dose)
        else:
            dose = None

        data, seg, dose = self.run_case_npy(data, seg, dose, data_properties, plans_manager, configuration_manager, dataset_json)
        # print_nonzero_elements(dose)
        return data, seg, dose, data_properties

    def run_case_save(self, output_filename_truncated: str, image_files: List[str], seg_file: str, dose_file: str,
                      plans_manager: PlansManager, configuration_manager: ConfigurationManager,
                      dataset_json: Union[dict, str]):
        # print(dose_file)
        data, seg, dose, properties = self.run_case(image_files, seg_file, dose_file, plans_manager, configuration_manager, dataset_json)
        # print(data.dtype)
        # print(dose)
        # print(properties)
        # print('dtypes', data.dtype, seg.dtype)
        np.savez_compressed(output_filename_truncated + '.npz', data=data, seg=seg, dose=dose)
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes) as p:
            for k in dataset.keys():
                r.append(p.starmap_async(self.run_case_save,
                                         ((join(output_directory, k), dataset[k]['images'], dataset[k]['label'], dataset[k]['dose'],
