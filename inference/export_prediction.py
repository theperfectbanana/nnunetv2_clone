...
def convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits: Union[torch.Tensor, np.ndarray],
    ...
    # print('convert_predicted_logits_to_segmentation_with_correct_shape')
    ...
    segmentation_reverted_cropping = np.zeros(properties_dict['shape_before_cropping'],
                                              dtype=np.float32 if len(label_manager.foreground_labels) < 255 else np.uint16)    
    slicer = bounding_box_to_slice(properties_dict['bbox_used_for_cropping'])
    segmentation_reverted_cropping[slicer] = segmentation
    # print(segmentation_reverted_cropping[slicer])
    del segmentation

    # revert transpose
    segmentation_reverted_cropping = segmentation_reverted_cropping.transpose(plans_manager.transpose_backward)
    # print(segmentation_reverted_cropping)
...
def resample_and_save(predicted: Union[torch.Tensor, np.ndarray], target_shape: List[int], output_file: str,
    ...
    print('resample_and_save')
    np.savez_compressed(output_file, seg=segmentation.astype(np.float32))
    torch.set_num_threads(old_threads)
