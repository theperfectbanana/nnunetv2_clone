...
#This is a hacky way to create dose_list_of_lists. 
def generate_dose_list_of_lists(list_of_lists):
    dose_list_of_lists = []
    for sublist in list_of_lists:
        dose_sublist = []
        for path in sublist:
            # replace imagesTr as customTr
            new_path = path.replace('imagesTs', 'customTs')
            # remove _0000
            dir_name, file_name = os.path.split(new_path)
            file_name = file_name.replace('_0000', '')
            new_path = os.path.join(dir_name, file_name)
            dose_sublist.append(new_path)
        dose_list_of_lists.append(dose_sublist)
        print("working")
    return dose_list_of_lists
...
def preprocess_fromfiles_save_to_queue(list_of_lists: List[List[str]],
                                       ...
                                       abort_event: Event,
                                       verbose: bool = False):
    #Generate dose list from image path list
    dose_list_of_lists = generate_dose_list_of_lists(list_of_lists)
    print(dose_list_of_lists)
