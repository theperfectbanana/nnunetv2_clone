...
def get_filenames_of_train_images_and_targets(raw_dataset_folder: str, dataset_json: dict = None):
    ...
    if 'dataset' in dataset_json.keys():
        ...
        for k in dataset.keys():
            ...
            dataset[k]['dose'] = abspath(join(raw_dataset_folder, dataset[k]['dose'])) if not os.path.isabs(dataset[k]['dose']) else dataset[k]['dose']
    else:
        ...
        doses = [join(raw_dataset_folder, 'customTr', i + dataset_json['file_ending']) for i in identifiers]
        # print(doses)
        dataset = {i: {'images': im, 'label': se, 'dose': ds} for i, im, se, ds in zip(identifiers, images, segs, doses)}
        # print(dataset)
    return dataset
