...
class nnUNetDataset(object):
    ...
    def load_case(self, key):
        ...
        #add dose file read in here
        if 'open_dose_file' in entry.keys():
            dose = entry['open_dose_file']
            # print('using open dose file')
        elif isfile(entry['data_file'][:-4] + "_dose.npy"):
            dose = np.load(entry['data_file'][:-4] + "_dose.npy", 'r')
            if self.keep_files_open:
                self.dataset[key]['open_dose_file'] = dose
                # print('saving open dose file')
        else:
            dose = np.load(entry['data_file'])['dose']
        ...
        return data, seg, dose, entry['properties']
