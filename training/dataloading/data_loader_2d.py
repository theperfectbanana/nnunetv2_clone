...
#         # dose_all = np.zeros(self.seg_shape, dtype=np.float32)
...
#             data, seg, dose, properties = self._data.load_case(current_key)
...
class nnUNetDataLoader2D(nnUNetDataLoaderBase):
    def generate_train_batch(self):
        ...
        for j, current_key in enumerate(selected_keys):
            ...
            this_slice = tuple([slice(0, dose.shape[0])] + [slice(i, j) for i, j in zip(valid_bbox_lbs, valid_bbox_ubs)])
            dose = dose[this_slice]

            padding = [(-min(0, bbox_lbs[i]), max(bbox_ubs[i] - shape[i], 0)) for i in range(dim)]
            data_all[j] = np.pad(data, ((0, 0), *padding), 'constant', constant_values=0)
            seg_all[j] = np.pad(seg, ((0, 0), *padding), 'constant', constant_values=-1)
            dose_all[j] = np.pad(dose, ((0, 0), *padding), 'constant', constant_values=0)

        return {'data': data_all, 'seg': seg_all, 'dose': dose_all, 'properties': case_properties, 'keys': selected_keys}
